<?php
require 'config.php';
if (is_admin_logged_in()) { header('Location: admin_dashboard.php'); exit; }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$username || !$password) { $err = "Semua field harus diisi."; }
    else {
        $stmt = $conn->prepare("SELECT id, password FROM admins WHERE username = ?");
        $stmt->bind_param('s', $username);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows == 0) { $err = "Admin tidak ditemukan."; }
        else {
